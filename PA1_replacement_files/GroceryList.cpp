...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  int FindItem(std::vector<std::string>g, std::string GroceryItem){
    ...
    return it-_gList_vector.begin();
  }


  results = FindItem(_gList_vector, groceryItem);

  if(results == size()){
    std::cout << size() << std::endl;
  }else{
    std::cout << "Item found at: " << results << std::endl;
  }
  /////////////////////// END-TO-DO (2) ////////////////////////////
}
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      if( !(_gList_array_size < _gList_array.size())) throw CapacityExceeded_Ex("Array capacity reached" exception_location); 
		
      for (int i = size() - 1; i > 0; i--)	
        _gList_array[i] = _gList_array[i - 1];		
      
      _gList_array[0] = groceryItem;
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ///////////////////////// TO-DO (5) //////////////////////////////
      ...
      _gList_vector.insert(beginItr,groceryItem);
